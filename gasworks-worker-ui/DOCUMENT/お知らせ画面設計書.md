...
## 0. 本設計書の作成プロセス（受講生向け）
本設計書は、プロトタイプのHTMLを基にAI（Gemini等）との対話を通じて作成されています。以下の手順で再現可能です。
1. **UI解析**: HTML/CSSをAIに読み込ませ、画面要素を抽出させる。
2. **データモデル定義**: 抽出要素からEntity（データ構造）と機能仕様を定義させる。
3. **アーキテクチャ設計**: Controller/Repositoryの役割とモックデータを生成させる。
4. **ドキュメント化**: 構成ファイル一覧を含めたMarkdown形式に整理させる。
